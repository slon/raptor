...
blob_t wire_reader_t::raw(int32_t size) {
    if(size + pos_ > blob_.size()) {
        throw exception_t("blob_t wire_reader_t::raw(int32_t) overflow");
    }

    blob_t blob = blob_.slice(pos_, size);
    pos_ += size;
    return blob;
}
...
void wire_reader_t::raw(int32_t size, char* buffer) {
    ...
    memcpy(buffer, blob_.data() + pos_, size);
    pos_ += size;
}
